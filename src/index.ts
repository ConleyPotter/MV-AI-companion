...
async function fetchLatestEcho(notionToken: string, dbId: string) {
  ...
  const tags = page.properties.Tags?.multi_select?.map((t: any) => t.name).join(", ") || "";
  return { id: page.id, content, tags };
}

function generatePrompt({ content, tags }: { content: string; tags: string }) {
...
async function getReflection(prompt: string, apiKey: string) {
  const res = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      model: "gpt-3.5-turbo",
      messages: [
        { role: "system", content: "You are a poetic and emotionally intelligent reflection companion." },
...
async function writeReflectionToNotion(token: string, pageId: string, reflection: string) {
